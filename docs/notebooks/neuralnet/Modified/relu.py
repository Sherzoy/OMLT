...
def bigm_leaky_relu_activation_constraint(net_block, net, layer_block, layer):
    ...
    layer_block.c5=pyo.Constraint(layer.output_indexes)
    layer_block.c6=pyo.Constraint(layer.output_indexes)
    layer_block.lbb=pyo.Param(layer.output_indexes,default=-1e6,mutable=True)
    layer_block.ubb=pyo.Param(layer.output_indexes,default=1e6,mutable=True)
    #integrate
    for output_index in layer.output_indexes:
        lb,ub = layer_block.zhat[output_index].bounds
        layer_block.lbb[output_index]=lb
        layer_block.z[output_index].setlb(max(0.01*lb,lb))
        layer_block.ubb[output_index]=ub
        layer_block.z[output_index].setub(max(0.01*ub,ub))
        q=layer_block.qb[output_index]
        zhat=layer_block.zhat[output_index]
        z=layer_block.z[output_index]
        layer_block.c1[output_index]=z>=0.01*zhat
        layer_block.c2[output_index]=z>=zhat
        layer_block.c3[output_index]=z<=zhat-lb(1-q)
        layer_block.c4[output_index]=z<=0.01*zhat+ub*q
        layer_block.c5[output_index]=zhat<=ub*q
        layer_block.c6[output_index]=zhat>lb*(1-q)
